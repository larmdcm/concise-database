...
trait Relation
{
	...
	public function hasOne ($modelName,$foreignKey = '',$primaryKey = 'id')
	{
		return new HasOne(
			$this,
			new $modelName,
			$this->getRelationModelKey(basename(str_replace("\\","/",get_class($this))),$foreignKey),
			$this->getRelationPrimaryKey($primaryKey)
		);
	}
	...
	public function belongsTo ($modelName,$foreignKey = '',$primaryKey = 'id')
	{
		return new BelongsTo($this,
			new $modelName,
			$this->getRelationModelKey($modelName,$foreignKey),
			$this->getRelationPrimaryKey($primaryKey)
		);
	}
	...
	public function belongsToMany ($modelName,$middleModelName,$foreignKey = '',$relationKey = '')
	{
		return new BelongsToMany(
			...
			new $middleModelName,
			$this->getRelationModelKey($modelName,$foreignKey),
			$this->getRelationModelKey(basename(str_replace("\\","/",get_class($this))),$relationKey)
		);
	}
	...
	protected function getRelationModelKey ($name,$foreignKey = '')
	{
		...
		$modelName = basename(str_replace("\\","/",$name));
