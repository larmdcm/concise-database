...
{	
	...
	public function parseWhereArray ($condtion)
	{
		...
		return $this->createSqlPrepare($this->parseField(array_keys($condtion),true,'=',' AND '),array_values($condtion));
	}
	 ...
	 * @return string
	 */
	public function parseField ($data,$comb = false,$exp = '=',$delimiter = ',')
	{
		$fields = '';
		array_walk($data,function ($value) use (&$fields,$comb,$exp,$delimiter) {
			$fields .= ($comb ? sprintf("%s %s %s",$this->escape($value),$exp,'?') : $this->escape($value)) . $delimiter;
		});
		return rtrim($fields,$delimiter);
	}
